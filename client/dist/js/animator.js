/* scroll-out v2.2.12 https://github.com/scroll-out/scroll-out MIT license */
...
/* animator.js */
disableElementAnimationAboveViewport();

ScrollOut({
    targets: '[data-animated]',
    once: true,
    threshold(el, ctx) {
        let thresholdPx = ctx.elementHeight * 0.2;
        if(thresholdPx > 200 || window.innerHeight < ctx.elementHeight) {
            return Math.min(200 / ctx.elementHeight, (window.innerHeight * 0.2) / ctx.elementHeight);
        } else {
            return 0.2;
        }
    },
    onShown(el) {
        if(el.dataset.animatedDisabled) return;

        window.setTimeout(function() {
            startElementAnimation(el);
        }, el.dataset.animatedDelay ? parseInt(el.dataset.animatedDelay) : 10);
    }
});

function startElementAnimation(element) {
    element.classList.add('animate__animated');
    element.classList.add('animate__' + element.dataset.animated);
    element.style.animationDelay = 0;

    if(element.dataset.animatedDuration) {
        element.style.animationDuration = element.dataset.animatedDuration + 'ms';
    }
}

function disableElementAnimationAboveViewport() {
    const animated = document.querySelectorAll('[data-animated]');
    for(let i = 0; i < animated.length; i++) {
        let rect = animated[i].getBoundingClientRect();

        // rect.bottom < 0 means that the bottom of the element is above the current screen
        if(rect && rect.bottom < 0) {
            animated[i].dataset.animatedDisabled = true;
        }
    }
}
